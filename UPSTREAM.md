...
| 2026-08-09 | (bundle + probes) | account refresh is `forceRefreshAllAccounts($input: ForceRefreshAllAccountsInput{source})` — refreshes ALL accounts (no per-account arg), poll `forceRefreshOperation(id){state,completed/totalAccountCount}`; goal create is `createSavingsGoals($input{goals: [CreateSavingsGoalInput!]!})` where each goal REQUIRES `name` AND `type` (objective) — payload has NO errors field; archive/create/delete work on savings-goal ids but `markGoalComplete/Incomplete` target the legacy Goal model (not-found for savings goals, so omitted) | added to writes.go accounts.go |
